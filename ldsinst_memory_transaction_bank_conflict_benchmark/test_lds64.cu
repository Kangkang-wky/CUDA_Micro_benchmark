...
// half-warp 先访问一半试试, 一半线程活跃, 16线程 x lds64(8 bytes) = 128 bytes = 1 个 memory transaction
...
// 两个 half-warp, 访问的是不同的 cacheline, memory transaction 内是没有 bank conflict 的
// 两次 memory transaction, 并且是没有 bank conflict 
